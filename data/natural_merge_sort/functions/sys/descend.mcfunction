...
execute store result score $Value1 _ run data get storage natural_merge_sort: ListAsc[-1][-1]
execute store result score $Value2 _ run data get storage natural_merge_sort: ListAsc[-2][-1]
execute unless data storage natural_merge_sort: ListAsc[-2][-1] run scoreboard players set $Value2 _ -2147483648
